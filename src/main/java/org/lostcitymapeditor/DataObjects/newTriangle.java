...
public class newTriangle {
    ...
    public static void addTriangle(boolean isModel, int x, int z, int level, int shape, int rotation, float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3, int color1, int color2, int color3, int textureId, float[] textureCoordinates) {
            if(color1 == 12345678)
                return;
            newTriangle triangle = new newTriangle();
            triangle.isModel = isModel;
            triangle.tileX = x;
            triangle.tileZ = z;
